<?php # PropServ Landlord Repair Requests - landlord_repair_requests.php
require('includes/config.inc.php');
$page_title = "Landlord Repair Requests";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	$q = "SELECT u.first_name, u.last_name, r.repair_id, r.repair_type, r.repair_comment, r.date_submitted FROM users u INNER JOIN repair r on u.user_id = r.user_id WHERE r.landlord_approval = 1 AND r.approved = 0 AND r.denied = 0";
	$r = mysqli_query($dbc, $q) or trigger_error("Query: $q\n<br>MySQL Error: " . mysqli_error($dbc));
	...
	if (@mysqli_num_rows($r) > 0) {
		...
		while ($repairs = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
			...
			// Provide dropdown with 2 options. One to approve the request and one to deny the request.
			echo '
			<form action="landlord_repair_requests.php" method="post">
	...
	} else { // If there are no repair requests requiring landlord approval
